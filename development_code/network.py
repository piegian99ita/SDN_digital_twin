...
def read_traffic(host,events): 
    ...
    while not host.shell or host.waiting:
        sleep(1)  
    if str(host)=="h1":
        print("READING-FLOW 3")
        events[2].set()
        host.cmd("timeout 20 tcpdump -i h1-eth0  not ether proto 0x88cc and not icmp6 -w h1.pcap ")
        
          
    elif str(host)=="h2":
        print("READING-FLOW 1")
        events[0].set()
        host.cmd("timeout 20 tcpdump -i h2-eth0  not ether proto 0x88cc and not icmp6 -w h2.pcap ")
        
        
    elif str(host)=="h3":
        print("READING-FLOW 2")
        events[1].set()
        host.cmd("timeout 20 tcpdump -i h3-eth0  not ether proto 0x88cc and not icmp6 -w h3.pcap ")
...
if __name__ == "__main__":
    ...
    for host in hosts_list:
        read_threads.append(threading.Thread(target=read_traffic,args=(host,events,)))
        traffic_threads.append(threading.Thread(target=generate_traffic,args=(host,events,)))
    
    for i in range(len(hosts_list)):
        print("start "+str(i))
        traffic_threads[i].start()
    ...
    send_thread = threading.Thread(target=send_pcap_data, args=(hosts_list,))
    send_thread.start()
