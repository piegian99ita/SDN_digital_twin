...
def remove_link(net, node1, node2):
    # print(f'*** Removing link between {node1} and {node2}\n')
    # link=net.linksBetween("h3","s3")
    # print(link)
    # net.delLink(link)
    # #net.delLinkBetween(node1, node2)
    i=0
    for links in net.links:
        if i==0:
            print("cancellazione del link--------------")
            net.delLink(links)
            
        i+=1
        print(links)

def delayed_removal(net, delay, host_name,switch_name):
    time.sleep(delay)
    remove_link(net, host_name, switch_name)
...
if __name__ == "__main__":
    ...
    removal_thread = threading.Thread(target=delayed_removal, args=(net, 5, 'h3','s3'))
